...
def init_subscribers():
  ...
  id = rospy.get_param("~id", 0) # id of this robot
  my_ground_truth_topic = '/robot_' + str(id) + '/base_pose_ground_truth'
  my_obstacles_topic = '/robot_' + str(id) + '/move_base/TebLocalPlannerROS/obstacles'
 
  obst_pub = rospy.Publisher(my_obstacles_topic, ObstacleArrayMsg, queue_size = 10)

  # Setup a subscriber for each obstacle -> get obstacle ids from published topics
  rospy.sleep(5) # wait until all nodes started and all topics published
  published_topics, published_types = zip(*rospy.get_published_topics())
  r = re.compile("^\/robot_([0-9][0-9]*)\/base_pose_ground_truth$")
  bpgt_topics = list(filter(r.match, published_topics))
  bpgt_topics.remove(my_ground_truth_topic) # remove self from the list
